        ...
        for line in a_file:
            ...
            if stripped_line != '':
                ...
                if not os.path.exists(outputOGGFile):
                    ...
                    ttsString = stripped_line.replace("<ANY>", 'this player').replace("<i>", "").replace("</i>", "")
...
def create_earwax_audio_jet(base_dir, files):
    ...
    with open(jet_file_path, "w") as newEarwaxAudio:
        ...
        newEarwaxAudio.write('\t\t{"x":false,"name":"'+files[0]+'","short":"' +
                           files[0]+'","id":"'+files[0]+'","categories":["household"]}')
